...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 20, 2021 at 09:58 AM
...
CREATE TABLE `subject_mapping` (
  `subject_mapping_id` int(200) NOT NULL,
  `subject_id` int(200) NOT NULL,
  `faculty_email_id` varchar(200) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
ALTER TABLE `subject_mapping`
  ADD PRIMARY KEY (`subject_mapping_id`),
  ADD KEY `fk2` (`subject_id`),
  ADD KEY `faculty_email_id` (`faculty_email_id`);
...
--
ALTER TABLE `subject_mapping`
  ADD CONSTRAINT `fk2` FOREIGN KEY (`subject_id`) REFERENCES `subject` (`subject_id`),
  ADD CONSTRAINT `subject_mapping_ibfk_1` FOREIGN KEY (`faculty_email_id`) REFERENCES `faculty` (`faculty_email_id`);
